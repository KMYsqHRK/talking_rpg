...
class GUI:
    ...
    def _load_assets(self):
        ...
        if os.path.exists(village_path):
            ...
            self.village_img = pygame.transform.smoothscale(
                raw_village, (WINDOW.width, WINDOW.height))
        else:
        ...
        if os.path.exists(bg_path):
            ...
            self.bg_img = pygame.transform.smoothscale(
                raw_bg, (WINDOW.width, WINDOW.height))
        else:
            self.bg_img = None
        ...
        if os.path.exists(lodge_path):
            ...
            self.lodge_img = pygame.transform.smoothscale(
                raw_lodge, (WINDOW.width, WINDOW.height))
        else:
            self.lodge_img = None
        ...
        if os.path.exists(portrait_path):
            ...
            self.portrait_img = pygame.Surface((PORTRAIT.width, PORTRAIT.height))
            self.portrait_img.fill(C.wood_dark)
    ...
    def _draw_village_screen(self):
        ...
        # タイトル
        title = self.font_title.render("Village", True, C.white)
        ...
        for line in self._wrap_text(loc["desc"], self.font_body, 500):
            desc_surf = self.font_body.render(line, True, C.parchment)
    ...
    def _draw_location_screen(self, title: str, bg_img: Optional[pygame.Surface]):
        ...
        # タイトル
        t = self.font_title.render(title, True, C.white)
        self.screen.blit(t, (WINDOW.width // 2 - t.get_width() // 2,
                             WINDOW.height // 2 - 60))

        # 未実装メッセージ
        msg = self.font_body.render("Coming soon...", True, C.parchment_dark)
    ...
    def _draw_background(self):
        ...
        title = self.font_title.render("Tavern Recruitment", True, C.gold)
    ...
    def _draw_portrait(self):
        ...
        if self.character:
            ...
            q = self.font_title.render("?", True, C.grey)
    ...
    def _draw_character_info(self):
        ...
        name_surf = self.font_header.render(ch['name'], True, C.gold)
        self.screen.blit(name_surf, (x, y))
        y += 30

        job_str = f"{ch['job']}  ({ch['role']})"
        job_surf = self.font_body.render(job_str, True, C.parchment)
        ...
        for line in self._wrap_text(pers_str, self.font_small,
            ...
            surf = self.font_small.render(line, True, C.parchment_dark)
    ...
    def _draw_dialogue(self):
        ...
        for msg in self.messages:
            ...
            if msg['is_user']:
                ...
                bg_color = C.user_bg
                sp = self.font_small.render("You", True, C.parchment_dark)
                ...
                bg_color = C.npc_bg
                name = msg['speaker']
                sp = self.font_small.render(name, True, C.gold)
                if area_y <= msg_y + total_height <= area_y + area_h:
            ...
            for line in lines:
                txt = self.font_body.render(line, True, C.charcoal)
    ...
    def _draw_input_area(self):
        ...
        if can_type and self.input_text:
            txt = self.font_body.render(self.input_text, True, C.charcoal)
            self.screen.set_clip(input_rect.inflate(-8, -4))
            self.screen.blit(txt, (ix + 8, iy + 10))
            self.screen.set_clip(None)
        elif can_type:
            ph = self.font_body.render("Type your message...", True, C.grey)
    ...
    def _draw_verdict_overlay(self):
        ...
        if self.verdict_frame > 15:
            ...
            prob = self.font_header.render(prob_str, True, C.white)
            ...
            if self.character:
                ...
                ns = self.font_body.render(name_str, True, C.parchment)
                self.screen.blit(ns,
    ...
    def _draw_loading_screen(self):
        ...
        txt = self.font_title.render("Loading Phi-2 Model...", True, C.gold)
        self.screen.blit(txt, (WINDOW.width // 2 - txt.get_width() // 2,
                               WINDOW.height // 2 - 40))

        dots = "." * ((pygame.time.get_ticks() // 500) % 4)
        d = self.font_header.render(dots, True, C.parchment)
